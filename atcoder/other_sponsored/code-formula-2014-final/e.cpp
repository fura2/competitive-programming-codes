...
int main(){
	int fib[23]={0,1};
	for(int i=2;i<=22;i++) fib[i]=fib[i-1]+fib[i-2];

	string s; cin>>s;
	int n;
	for(n=1;s.length()!=fib[n];n++);
	...
	vector<rolling_hash::hash_type> R[23];
	R[1].emplace_back(rolling_hash::get_hash("b"));
	R[2].emplace_back(rolling_hash::get_hash("a"));
	for(int i=3;i<=n;i++){
		R[i].resize(1<<(i-2));
		rep(k,1<<(i-2)){
			if(k%2==0) R[i][k]=R[i-1][k/2]*R[i-2][k/4];
			else       R[i][k]=R[i-2][k/4]*R[i-1][k/2];
		}
	}

	auto tar=rolling_hash::get_hash(s);
	rep(k,1<<(n-2)) if(R[n][k]==tar) {
		printf("%d %d\n",n,k);
		break;
	}
