	...
	rep(i,n-4){
		rep(j,16){
			rep(par,2){
				// 仕切りを入れる
				if(j<15 && (j>=5 || par==0)){
					dp[i][j+1][0]+=dp[i][j][par];
				}
				// ボールを置く
				if(i<n-5){
					dp[i+1][j][1-par]+=dp[i][j][par];
				}
			}
		}
	}
	cout<<dp[n-5][15][0]+dp[n-5][15][1]<<'\n';
*/
	matrix<mint> A(32); // 仕切りを 1 枚追加する
	rep(j,16) rep(par,2) {
		if(j<15 && (j>=5 || par==0)){
			A[j+1][par*16+j]+=1;
		}
	}
	auto A2=matrix<mint>::identity(32); // 仕切りを任意の枚数追加する
	rep(i,15){
		A2=A2*A+matrix<mint>::identity(32);
	}
	matrix<mint> B(32); // ボールを 1 個置く
	rep(j,16) rep(par,2) {
		B[(1-par)*16+j][par*16+j]+=1;
	}

	auto C=A2*pow(B*A2,n-5);
	cout<<C[15][0]+C[31][0]<<'\n';
}
