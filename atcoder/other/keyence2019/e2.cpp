...
int main(){
	...
	rep(i,n){
		SL.update(i,make_pair(a[i]-d*i,i));
		SR.update(i,make_pair(a[i]+d*i,i));
	}
	...
	auto f=[&](auto&& f,int l,int r){
		...
		int i=(l+r)/2;
		if(l<i){
			int u_opt=SL.product(l,i).get().second;
			for(int v=i;v<r;v++){
				E.emplace_back(v,u_opt,a[v]+a[u_opt]+d*abs(v-u_opt));
			}
		}
		if(i<r){
			int v_opt=SR.product(i,r).get().second;
			for(int u=l;u<i;u++){
				E.emplace_back(u,v_opt,a[u]+a[v_opt]+d*abs(u-v_opt));
			}
		}
		f(f,l,i);
		f(f,i,r);
