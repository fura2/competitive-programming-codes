...
int main() {
    int d;
    string a, b; cin >> d >> a >> b;

    if (a.size() < b.size()) swap(a, b);
    b = string(a.size() - b.size(), '0') + b;
    int n = a.size();

    string a1 = a.substr(0, n - 1 - d), a2 = a.substr(n - d);
    string b1 = b.substr(0, n - 1 - d), b2 = b.substr(n - d);

    string ans;
    int carry = 0;
    rrep(i, d) {
        int x = (a2[i] - '0') + (b2[i] - '0') + carry;
        ans += (x % 10) + '0';
        carry = x / 10;
    }
    ans += '.';
    rrep(i, n - 1 - d) {
        int x = (a1[i] - '0') + (b1[i] - '0') + carry;
        ans += (x % 10) + '0';
        carry = x / 10;
    }
