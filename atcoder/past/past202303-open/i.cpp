...
pair<int, char> score(string t) {
    int hist[26] = {};
    rep(i, 5) hist[t[i] - 'A']++;
    int idx = max_element(all(hist)) - hist;
    return { -hist[idx], 'A' + idx };
}

pair<int, char> score(string x, string s) {
    pair<int, char> res = { 0, 'A' };
    string t(5, '?');
    rep(i1, 4) rep(i2, i1 + 1, 4) {
        t[0] = s[i1];
        t[1] = s[i2];
        rep(j1, 5) rep(j2, j1 + 1, 5) rep(j3, j2 + 1, 5) {
            t[2] = x[j1];
            t[3] = x[j2];
            t[4] = x[j3];
            res = min(res, score(t));
...
int main() {
    ...
    vector<tuple<int, char, int>> p(n);
    rep(i, n) {
        auto [m, c] = score(x, s[i]);
        p[i] = { m, c, i };
    }
    cout << get<2>(*min_element(all(p))) + 1 << endl;
