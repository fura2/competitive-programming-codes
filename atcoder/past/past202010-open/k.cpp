...
int main(){
	int k; scanf("%d",&k);
	vector<int> n(k);
	vector a(k,vector<int>());
	rep(i,k){
		scanf("%d",&n[i]);
		a[i].resize(n[i]);
		rep(j,n[i]) scanf("%d",&a[i][j]), a[i][j]--;
	}

	vector<lint> inv(k);
	vector freq(k,vector<mint>(20));
	rep(i,k){
		inv[i]=inversion_number(a[i]);
		rep(j,n[i]) freq[i][a[i][j]]+=1;
	}
	...
	rep(_,q){
		...
		ans+=inv[idx];
		rep(x,20) for(int y=x+1;y<20;y++) ans+=freq[idx][x]*cnt[y];

		rep(x,20) cnt[x]+=freq[idx][x];
	}
	printf("%lld\n",ans);
