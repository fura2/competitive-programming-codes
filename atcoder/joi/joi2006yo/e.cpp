...
int main(){
	...
	if(m==0){
		...
		if(n<=2){
			...
			bint x("1"+string(r+100,'0'));
			rep(i,n-1){
				bint y("1"+string(r+100,'0'));
				y/=i+1;
				x+=y;
			}
			x/=n;
			string s=to_string(x);
			cout<<"0."<<string(r+100-s.length(),'0')<<s.substr(0,s.length()-100)<<'\n';
		}
	}
